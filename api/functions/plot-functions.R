...
make_publications_plot <- function(data_tibble) {
  plot <- ggplot(data = data_tibble,
      aes(x = publication_date,
      color = publication_type)) +
    stat_bin(data=subset(data_tibble,
    ...
    stat_bin(data=subset(data_tibble,
      ...
      geom="step", bins = 30) +
    stat_bin(data=subset(data_tibble,
      publication_type == "screening_multiple"),
      aes(y=cumsum(..count..)), geom="step", bins = 30) +
    stat_bin(data=subset(data_tibble,
...
make_cohort_plot <- function(data_tibble) {
  ...
  plot_hnf1b_db_individual_sex <- ggplot(hnf1b_db_individual_sex,
    aes(ymax=ymax, ymin=ymin, xmax=4, xmin=3, fill=sex)) +
      geom_rect() +
  ...
  plot_hnf1b_db_individual_cohort <- ggplot(hnf1b_db_individual_cohort,
  ...
  plot_hnf1b_db_individual_mult <- ggplot(hnf1b_db_individual_multiple,
    aes(ymax=ymax, ymin=ymin, xmax=4, xmin=3,
    fill=reported_multiple)) +
      geom_rect() +
  ...
  cohort_plot <- plot_grid(plot_hnf1b_db_individual_sex,
    plot_hnf1b_db_individual_cohort,
    plot_hnf1b_db_individual_mult,
        ncol = 3, nrow = 1)
