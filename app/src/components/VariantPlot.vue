...
<script>
  ...
  data() {
        return {
          ...
          items: ['detection_method', 'segregation', 'class', 'impact', 'effect', 'classification'],
        }
      },
      computed: {
      },
      ...
      methods: {
        async loadVariantData() {
            ...
            let apiUrl = process.env.VUE_APP_API_URL + '/api/statistics/variant_characteristics';
        ...
        generateDonutGraph() {
          ...
          const svg = d3.select("#variant_dataviz")
            .append("svg")
          ...
          // Create a tooltip
          // ----------------
          const tooltip = d3.select("#variant_dataviz")
              .append("div")
              .style("opacity", 0)
              .attr("class", "tooltip")
