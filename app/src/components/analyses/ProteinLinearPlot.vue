  ...
  methods: {
    async loadDomainData() {
      ...
      const filter_variant_class =
        "any(variant_class," + this.variant_class_select.join(",") + ")";
      const filter_classification =
        "any(verdict_classification," +
        this.classification_select.join(",") +
        ")";
      const filter_expression =
        "&filter=" +
        filter_variant_class +
        "," +
      ...
      let apiUrlVariants =
        ...
        "/api/variants?sort=variant_id&fields=variant_id,variant_class,FEATUREID,HGVS_C,HGVS_P,IMPACT,EFFECT,Protein_position,CADD_PHRED,verdict_classification&page_after=0&page_size=all" +
        filter_expression;
    ...
    generateLinearProteinGraph(domain_input, variant_input) {
      ...
      // first remove svg
      d3.select("#protein_linear_dataviz").select("svg").remove();

      // append the svg object to the body of the page
      const svg = d3
        .select("#protein_linear_dataviz")
        .append("svg")
      ...
      const maxLengthScaled =
        Math.max(...domain_input.map((o) => o.length)) * length_factor;

      const variant_data = variant_input.map((obj) => {
        return { ...obj, Protein_position: obj.Protein_position };
      });
      ...
      const mouseleave = function (event, d) {
        ...
        d3.select(this).style("stroke-width", 1).style("stroke", "grey");
      };
      ...
      svg
        ...
        .append("line")
        .attr("x1", function (d) {
          return x(d.Protein_position);
        })
        .attr("x2", function (d) {
          return x(d.Protein_position);
        })
        .attr("y1", function (d) {
          return y(d.CADD_PHRED + 15);
        })
        .attr("y2", y(15))
        .attr("stroke", "grey");
      ...
      svg
        ...
        .text(function (d) {
          if (
            d.length / maxLengthScaled > 0.05 &&
            d.length / maxLengthScaled != 1
          ) {
            return d.description_short;
          }
        })
        .attr("transform", `translate(0,${height})`);
    },
