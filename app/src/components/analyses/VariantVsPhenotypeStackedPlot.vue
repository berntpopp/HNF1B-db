<template>
  <v-container fluid>
    ...
    <div id="phenotype_variant_dataviz" class="svg-container"></div>
    <!-- Content publications plot-->
  </v-container>
</template>
...
export default {
  name: "VariantVsPhenotypeStackedPlot",
  ...
  methods: {
    async loadData() {
      let apiUrl =
        process.env.VUE_APP_API_URL + "/api/statistics/variantattributes_vs_phenotypes";

      try {
        let response = await this.axios.get(apiUrl);

        this.itemsVP = response.data.data;
        this.itemsVPMeta = response.data.meta;

        this.generateVPGraph();
      } catch (e) {
        console.error(e);
      }
    },
    generateVPGraph() {
      // based on https://bl.ocks.org/SpaceActuary/6233700e7f443b719855a227f4749ee5
      const margin = { top: 10, right: 30, bottom: 150, left: 100 },
        width = 750 - margin.left - margin.right,
        height = 300 - margin.top - margin.bottom;

      // first remove svg
      d3.select("#phenotype_variant_dataviz").select("svg").remove();

      // append the svg object to the body of the page
      const svg_raw = d3
        .select("#phenotype_variant_dataviz")
        .append("svg")
      ...
      // List of groups
      const groups = data.map((d) => d.phenotype_name);
      const keys = data.map((d) => d.described);
